...
const int in1 = 19;  // IN1
const int in2 = 18;  // IN2
...
void setup() {
  ...
  server.on("/", HTTP_GET, [](AsyncWebServerRequest *request) {
    String response =
        ...
        "<button style='margin: 20px; padding: 10px 20px; font-size: 28px; "
        "background-color: "
        "#b82525; color: white; border: none; border-radius: 5px; cursor: "
        "pointer;' onclick='motorStop()'>Stop</button>"
        ...
        "function motorStop(){fetch('/stop').then(response => "
        "response.text()).then(data => console.log(data));}"
